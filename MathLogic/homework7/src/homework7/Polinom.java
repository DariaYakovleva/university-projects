...
public class Polinom implements Expression{
	public List<Mpair> polinom;
	public Const p;
//	public Mpair cnt;
//	public Polinom next;
	
	public Polinom() {
		polinom = new ArrayList<Mpair>();
		p = new Const(0);
	}
	
	public Polinom(List<Mpair> a, Const p) {
		this.p = new Const(p.x);
		polinom = new ArrayList<Mpair>();
		for (int i = 0; i < a.size(); i++) {
			polinom.add(new Mpair(a.get(i)));
		}
	}
	...
	public Polinom(Polinom a) {
		...
		polinom = new ArrayList<Mpair>();
		for (int i = 0; i < a.size(); i++) {
			polinom.add(new Mpair(a.polinom.get(i)));
		}
	}
	...
	void push(Expression power, Const mult) {
		polinom.add(new Mpair(mult, power.evaluate()));
	}
	...
	public Polinom add(Polinom a) {
		Polinom res = new Polinom(polinom, p);
	...
	public Polinom first() {
		if (polinom.isEmpty()) {
			return new Polinom(new ArrayList<Mpair>(), p);
		}

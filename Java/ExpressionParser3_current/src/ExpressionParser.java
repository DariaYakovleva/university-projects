...
public class ExpressionParser<T extends MyNumber<T>> {
	...
	private void lexem(String s1) {
		while (i < s1.length()) {
			...
			if (f) {
				s.add(a);
				i--;
			} else if ((s1.charAt(i) == 'x') || (s1.charAt(i) == 'y')
					|| (s1.charAt(i) == 'z')) {
				s.add("" + s1.charAt(i));
	...
	private Expression3<T> multiplier() throws ParseException {
		...
		if (ok(s.get(nextperm))) {
			a = new Const<T>(tmp.parse(s.get(nextperm)));
			nextperm++;
			if (ok(s.get(nextperm))) {
			...
			if ((s.get(nextperm).equals("x")) || (s.get(nextperm).equals("y"))
				...
				nextperm++;
			} else if (s.get(nextperm).equals(OPEN)) {
				nextperm++;
				a = expr();
				...
				nextperm++;
			} else if (s.get(nextperm).equals(LOG)) {
				nextperm++;
				a = new Log<T>(expr());
			} else if (s.get(nextperm).equals(SUB)) {
				nextperm++;
				a = new Minus<T>(poww());
			} else if (s.get(nextperm).equals(TIL)) {
				nextperm++;
				a = new Not<T>(poww());
			} else if (s.get(nextperm).equals(EXC)) {
				nextperm++;
				a = new Abs<T>(poww());
			} else if ((s.get(nextperm).equals("f"))) {
				// make_new_function
				// begin

				class Factorial<E extends MyNumber<E>> implements
						Expression3<T> {
					...
					Function giveMe() {
						if (type.equals("-i")) {
							...
							return new Function<MyDouble>() {
								public MyDouble function(MyDouble c) {
									double res = 1;
									for (int i = 1; i <= c.a; i++) {
										res *= i;
									}
									return new MyDouble(res);

								};
							};
						} else {
							return new Function<MyBigInteger>() {
								public MyBigInteger function(MyBigInteger c) {
									BigInteger res = BigInteger.valueOf(1);
									for (BigInteger i = BigInteger.valueOf(1); !i.equals(c.a); i.add(BigInteger.valueOf(1))) {
										res.multiply(i);
									}
					...
					public T evaluate(T x, T y, T z) throws Exception3 {
						T res = e1.evaluate(x, y, z);
						return (T) giveMe().function(res);
					}
	...
	private ExpressionParser(String a, T t, String b) throws ParseException {
		i = 0;
		tmp = t;
		type = b;
		nextperm = 0;
	...
	static public <E extends MyNumber<E>> Expression3<E> parse(String a, E t, String b) throws ParseException {
		return new ExpressionParser<E>(a, t, b).res;
	}
}

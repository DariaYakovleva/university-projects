...
public class GenericParser<T extends MyNumber<T>> {
		public static void main(String[] args) {
				...
				if (args[0].equals("-i")) {
						...
						Expression3<MyInteger> ans = ExpressionParser.parse(args[1], new MyInteger(1));
						for (Integer i = -100; i <= 100; i++) {
						for (Integer j = -100; j <= 100; j++) {
							try {
				...
				if (args[0].equals("-d")) {
					Expression3<MyDouble> ans = ExpressionParser.parse(args[1], new MyDouble(1.0));
				...
				if (args[0].equals("-bi")) {
					Expression3<MyBigInteger> ans = ExpressionParser.parse(args[1], new MyBigInteger(BigInteger.valueOf(1)));
